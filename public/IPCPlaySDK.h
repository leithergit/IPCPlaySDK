...
/// @param [in]		hPlayHandle	由ipcplay_OpenFile或ipcplay_OpenStream返回的播放句柄
/// @param [in]		pUserPtr	用户自定义指针
typedef void(__stdcall *VideoSwitchCallBack)(HWND hWnd,IPC_PLAYHANDLE hPlayerHandle, void *pUserPtr);

struct VideoSwitchInfo
{
	
	VideoSwitchInfo(HWND hWnd,void *pPlayer,void *pCallBack, void *pUserPtr)
	{
		this->hWnd = hWnd;
		pPlayerHandle = pPlayer;
		this->pUserPtr = pUserPtr;
		this->pCallBack = (VideoSwitchCallBack)pCallBack;
	}
	void Reset()
	{
		ZeroMemory(&hWnd, sizeof(VideoSwitchInfo) - offsetof(VideoSwitchInfo, hWnd));
	}
	~VideoSwitchInfo()
	{
		if (pCallBack)
...
/// #- true			开启硬解码功能
/// #- false		关闭硬解码功能
/// @param [in]		pVideoSwitchCB	当前视频画面即将进入解码阶段时，该回调将被调用，便于调用者作一些其它工作
/// @remark			该回调可用于实现无延时画面切换，当前窗口正在显示视频画面时，若要切入另一画面，便可设置此回调，在回调中关闭前一视频
/// @retval			0	操作成功
/// @retval			-1	输入参数无效
/// @remark			当开启硬解码，而显卡不支持对应的视频编码的解码时，会自动切换到软件解码的状态,可通过此回调关闭前视频画面，以实现无延时切换
///					ipcplay_GetHaccelStatus判断是否已经开启硬解码
IPCPLAYSDK_API int ipcplay_Start(IN IPC_PLAYHANDLE hPlayHandle, IN bool bEnableAudio = false, bool bFitWindow = true, bool bEnableHaccel = false,void *pVideoSwitchCB = nullptr, void *pUserPtr = nullptr);
...
IPCPLAYSDK_API int ipcplay_AddWindow(IN IPC_PLAYHANDLE hPlayHandle, HWND hRenderWnd, LPRECT pRectRenderBorder = nullptr, bool bPercent = false,void *pVideoSwitchCB = nullptr, void *pUserPtr = nullptr);
