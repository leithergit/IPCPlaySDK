...
// 读取硬盘序列号函数    
bool CHardwareID::GetHDSerial(char *szHDSerial,int nSize)   
{      
	m_buffer[0]='\n';   
	// 得到当前操作系统版本    
	...
	{   
		...
		if (!WinNTReadIDEHDSerial(m_wWinNTHDSerial))    
			 if (!WinNTReadSCSIHDSerial(m_wWinNTHDSerial))
				 return false;
		strcpy_s(szHDSerial, nSize, DWORDToChar (m_wWinNTHDSerial, 10, 19));   
	}    
	return true;
}   

// Windows9X/ME系统下读取硬盘序列号    
void _stdcall CHardwareID::Win9xReadHDSerial(WORD * buffer)   
{      
	int i;         
	for(i=0;i<256;i++)    
		buffer[i]=0;   
...
char * CHardwareID::WORDToChar (WORD diskdata [256], int firstIndex, int lastIndex)   
{   
...
char* CHardwareID::DWORDToChar (DWORD diskdata [256], int firstIndex, int lastIndex)   
{   
	static char string [1024];   
	int index = 0;   
	int position = 0;   
...
BOOL CHardwareID::WinNTReadIDEHDSerial(DWORD * buffer)   
{   
...
BOOL CHardwareID::WinNTReadSCSIHDSerial (DWORD * buffer)   
{      
	buffer[0]='\n';   
	int controller = 0;   
...
BOOL CHardwareID::WinNTGetIDEHDInfo (HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP,   
...
BOOL CHardwareID::GetCPUID(UINT64 *nCPUID,CHAR* szVenderID,int nVenderSize)
{
#define MAKEUINT64(low,high) ((UINT64)(((UINT)(low)) | ((UINT64)((UINT)(high))) << 32))
	unsigned long s1,s2;
	unsigned char szVenderID2[32] = {0};
	UINT64	nCPUID2;
	bool bExcptions = false;
	__try
	{
		__asm
		{
			xor eax,eax      //eax=0:取Vendor信息
				...
				mov dword ptr szVenderID2,ebx
				mov dword ptr szVenderID2[+4],edx
				mov dword ptr szVenderID2[+8],ecx
		...
		nCPUID2 = MAKEUINT64(s1,s2);
		//sprintf_s((char *)szCPUID2,128,"%08X%08X",s1,s2);
		return true;
	}
	__except(1)
	{
	...
	{
		*nCPUID = nCPUID2;
		strcpy_s(szVenderID,nVenderSize,(char *)szVenderID2);
	}
	return FALSE;
}