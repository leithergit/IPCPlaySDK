// IPCPlayDemoWin32.cpp : Defines the entry point for the application.
//
...
#endif
typedef void (CALLBACK* fnIPCCallback_RealAVData_T)( USER_HANDLE lUserID,  REAL_HANDLE lStreamHandle,  int nErrorType,  const char* pBuf,  int nDataSize,  void* pUser);

/// @brief		解码后YVU数据回调
void __stdcall _CaptureYUV(IPC_PLAYHANDLE hPlayHandle,
...
public:
	PlayerContext(USER_HANDLE hUserIn,
		...
		IPC_PLAYHANDLE hPlayerIn = nullptr, int nCount = 1)
	{
                     ...
                     _In_ int       nCmdShow)
{
	...
	hInst = hInstance;
	InitCommonControls();
	int nResult = DialogBox(hInst, MAKEINTRESOURCE(IDD_IPCIPCPLAY), NULL, (DLGPROC)DialogProc);
...
// 	int nError = 0;
// 	IPC_MEDIAINFO MediaHeader;
// 	IPC_PLAYHANDLE hPlayer = nullptr;
// 	// 设置图像尺寸和音视频编码
// 	MediaHeader.nVideoWidth = 1280;
// 	MediaHeader.nVideoHeight = 720;
...
// 	hStreamHandle = IPC2_NET_StartRealPlay(hUser,
// 		0,
// 		nStream,
// 		IPC_TCP,
// 		0,
...
void OnButtonStopPlay(HWND hDlg)
{
	...
	for (int i = 0; i < g_vIPCamera.size(); i++)
		if (m_pPlayContext[i])
		{
			IPC2_NET_StopRealPlay(m_pPlayContext[i]->hStream);
			m_pPlayContext[i]->hStream = -1;
			EnableDlgItem(hDlg, IDC_COMBO_STREAM, TRUE);
			if (m_pPlayContext[i]->hPlayer)
			{
	...
	case ID_TIMER_CHECKSTREAM:
		{
			if (m_pPlayContext[0] && m_pPlayContext[0]->hPlayer)
			{
				int nError = 0;
				int nStream = SendDlgItemMessage(hDlg, IDC_COMBO_STREAM, CB_GETCURSEL, 0, 0);
				IPC2_NET_StopRealPlay(m_pPlayContext[0]->hStream);
				// 重新打开码流
				m_pPlayContext[0]->hStream = IPC2_NET_StartRealPlay(m_pPlayContext[0]->hUser,
					0,
					nStream,
					IPC_TCP,
					0,
...
IPC_PLAYHANDLE	g_hFilePlay = nullptr;
